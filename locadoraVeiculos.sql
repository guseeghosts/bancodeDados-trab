...
select sum(ValorTotal) as 'Valor Total Arrecadado' from Pagamento where estado = 'Pago';

select V.modelo, V.marca, count(LV.idLocacao) as `Numero De Alugueis` from Veiculo as V join LocacaoVeiculo as LV on V.idVeiculo = LV.idVeiculo group by V.modelo, V.marca order by `Numero De Alugueis` desc;

select C.nome as `Nome do Cliente`, sum(P.valorTotal) as `Valor Devido` from Cliente as C join Locacao as L on C.idCliente = L.idCliente join Pagamento as P on L.idPagamento = P.idPagamento where P.estado = 'Pendente' group by C.nome order by C.nome asc;